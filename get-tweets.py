...
hashtags_list = ['#openbanking','#apifirst','#devops','#cloudfirst','#microservices','#apigateway','#oauth','#swagger','#raml','#openapis']
...
for hashtags in hashtags_list:
    print("Hashtag: %s \n" % (hashtags))
    results = twitter.search(q="%s" % (hashtags))
    for tweet in results:
        user = tweet.user.screen_name
        followers = tweet.user.followers_count
        message = tweet.text
    print("User: %s - Followers: %s - Tweet: %s \n" % (user, followers, message))
